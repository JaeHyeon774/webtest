...
public class BbsDAO {
	...
	public boolean createReply(BbsDTO dto) {
		...
		try {
			...
			pstmt.setInt(5, dto.getGrpno()); // ¡Ú ºÎ¸ðÀÇ grpno
			pstmt.setInt(6, dto.getIndent()+1); // ¡Ú ºÎ¸ðÀÇ indent + 1
			pstmt.setInt(7, dto.getAnsnum()+1); // ¡Ú ºÎ¸ðÀÇ ansnum + 1
	...
	public BbsDTO readRelpy(int bbsno) {
		BbsDTO dto = null;
		Connection con = Open.getConnection();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public int total(String col, String word) {
		...
		if (word.trim().length() > 0 && col.equals("title_content")) {
			sql.append(" where title like '%'||?||'%' ");// =>'%¿Õ%' ¿Õ=>word
